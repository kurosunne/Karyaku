...
<body>
    ...
    <div class="container d-flex justify-content-center mt-3 flex-column">
        <div id="carouselExampleInterval" class="carousel slide" data-bs-ride="carousel" style="width: 90%; margin:auto; height:400px; border-radius:10px;">
        ...
        <!--List Item-->
        <h1 class="mt-3 mb-0" style="margin:auto;"></h1>
        <div style="width: 100%;" class="row">
            <?php
            $query = $koneksi->prepare("SELECT lp.*, NVL((SELECT CAST(SUM(h.rate)/count(h.rate) as INT) from history h where lp.product_id=h.product_id and h.rate!=0) ,'0') as 'rating' FROM list_product lp");
            $query->execute();
            $hasil = $query->get_result()->fetch_all(MYSQLI_ASSOC);

            foreach ($hasil as $key => $value) {
            ?>
                <div class="klik col-xxl-3 col-xl-4 col-lg-6 mb-xl-0 mb-xxl-0 mb-md-5" style="height: 420px;">
